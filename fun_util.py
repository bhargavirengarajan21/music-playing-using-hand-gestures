...
def get_pred_music_from_db(pred_class):
	conn = sqlite3.connect("getsure1")
	cmd = "SELECT music FROM gesture WHERE g_id="+str(pred_class)
...
def get_pred_from_contour(contour, thresh):
	...
	return text
def get_pred_from_contour1(contour, thresh):
	x1, y1, w1, h1 = cv2.boundingRect(contour)
	save_img = thresh[y1:y1+h1, x1:x1+w1]
	...
	pred_probab, pred_class = keras_predict(model, save_img)
	if pred_probab*100 > 90:
		file = get_pred_music_from_db(pred_class)
	return file
		...
		if len(contours) > 0:
			...
			if cv2.contourArea(contour) > 10000:
				...
				if count_same_frame > 5:
					       print(text)
                                               file= get_pred_from_contour1(contour, thresh)
                                               print(file)
                                               player = WavePlayerLoop(file) 
                                               player.play()
